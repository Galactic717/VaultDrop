...
(async()=>{
 ...
 const source=path.join(work,'Test files');const dest1=path.join(work,'copy-one');const dest2=path.join(work,'copy-two');
 fs.mkdirSync(path.join(source,'Photos, 2026'),{recursive:true});
 const fixtures={'notes.txt':Buffer.from('VaultDrop native release smoke test.\n'),'empty.txt':Buffer.alloc(0),'Photos, 2026/café ✓.bin':crypto.randomBytes(4*1024*1024+73)};
 ...
 await page.evaluate(({source,dest1,dest2})=>{
  localStorage.setItem('vaultdrop.lang',JSON.stringify('en'));
 ...
 await page.screenshot({path:path.join(root,'docs/screenshots/native-result-en.png'),fullPage:true});
 ...
 await page.screenshot({path:path.join(root,'docs/screenshots/native-damaged-en.png'),fullPage:true});
